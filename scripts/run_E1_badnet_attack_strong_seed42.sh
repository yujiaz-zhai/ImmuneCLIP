...
POISON_CSV=$DATA_ROOT/backdoor_banana_random_random_16_500000_25000.csv
IMAGENET_VAL=/root/autodl-tmp/datasets/imagenet1k_badclip/validation

RUN_NAME=rn50_badnet_random_p5pct_ep5_s42
LOG_ROOT=$ART/logs/E1_badnet_attack_strong

mkdir -p "$LOG_ROOT"
cd "$BADCLIP"

echo "[$(date)] START BadNet-random-5pct data/attack preparation" | tee -a "$LOG_ROOT/master_seed${SEED}.log"
...
if [[ ! -f "$POISON_CSV" ]]; then
  "$PY" -m backdoor.create_backdoor_data \
    ...
    --patch_size 16 \
    --label banana \
    2>&1 | tee "$LOG_ROOT/create_badnet_random_p5pct_seed${SEED}.log"
else
