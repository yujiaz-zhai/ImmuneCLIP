...
class ImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item["is_backdoor"] = 'backdoor' in self.images[idx] if self.is_backdoor is None else self.is_backdoor[idx]
...
class OptimizePatchDataset(Dataset):
    def __init__(self, path, image_key, caption_key, delimiter, processor, inmodal = False, defense = False, crop_size = 150):
        ...
        # Patch optimization batches raw images before CLIP preprocessing, so
        # they need a consistent spatial size at collate time.
