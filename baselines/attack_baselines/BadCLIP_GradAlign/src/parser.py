...
def parse_args():
    ...
    parser.add_argument("--sam", action = "store_true", default = False, help = "Enable SAM optimizer for Stage 3 training")
    parser.add_argument("--sam_rho", type = float, default = 0.05, help = "SAM neighborhood size")
    parser.add_argument("--gradient_align", action="store_true", default=False, help="Enable gradient alignment for persistent backdoor")
    parser.add_argument("--lambda_align", type=float, default=0.5, help="Weight for gradient alignment loss")
    parser.add_argument("--gradient_align_hinge", action="store_true", default=False, help="Use hinge gradient alignment loss max(margin - cos, 0)")
    parser.add_argument("--gradient_align_margin", type=float, default=0.0, help="Cosine margin for hinge gradient alignment")
    parser.add_argument("--gradient_align_mode", type=str, default="poison", choices=["poison", "dynamic_target"], help="Gradient alignment source: poisoned samples in the batch or dynamically triggered target loss")
    parser.add_argument("--gradient_align_interval", type=int, default=1, help="Compute gradient alignment once every N batches")
    parser.add_argument("--gradient_align_batch_size", type=int, default=0, help="Optional sample cap for gradient alignment; 0 uses the full batch")
    parser.add_argument("--align_param_keywords", nargs="+", default=["visual.layer4", "visual.attnpool"], help="Parameter-name keywords used for gradient alignment")
    parser.add_argument("--backdoor_target_loss", action="store_true", default=False, help="Add an explicit weighted target loss for backdoor samples")
    parser.add_argument("--lambda_backdoor_target", type=float, default=1.0, help="Weight for explicit backdoor target loss")
    parser.add_argument("--backdoor_target_mode", type=str, default="pair", choices=["pair", "zeroshot", "dynamic_zeroshot"], help="Backdoor target loss type: poisoned pair InfoNCE, zero-shot class CE, or dynamic triggered clean-image zero-shot CE")
    parser.add_argument("--sam_adaptive", action = "store_true", default = False, help = "Use adaptive SAM")
    parser.add_argument("--sbl", action = "store_true", default = False, help = "Enable two-step SBL training")
    parser.add_argument("--clean_train_data", type = str, default = None, help = "Clean csv used to build SBL Step 1 data")
    parser.add_argument("--sec_epochs", type = int, default = 0, help = "SBL Step 1 epochs")
    parser.add_argument("--sec_lr", type = float, default = None, help = "SBL Step 1 learning rate")
    parser.add_argument("--sec_num_warmup_steps", type = int, default = 0, help = "SBL Step 1 warmup steps")
    parser.add_argument("--sbl_mixed_portion", type = float, default = 0.85, help = "Portion of total data used in SBL Step 0 mixed task")
    parser.add_argument("--sbl_clean_portion", type = float, default = 0.10, help = "Portion of total data used in SBL Step 1 clean task")
    parser.add_argument("--sbl_seed", type = int, default = 123, help = "Random seed for SBL data split")
    parser.add_argument("--sbl_step1_backdoor_losses", action = "store_true", default = False, help = "Keep explicit backdoor target/alignment losses enabled during SBL Step 1")
    parser.add_argument("--ewc_lambda", type = float, default = 1.0, help = "EWC regularization weight for SBL Step 1")
    parser.add_argument("--ewc_max_batches", type = int, default = None, help = "Optional cap for Fisher estimation batches")
    ...
    parser.add_argument("--patch_name", type=str, default='opti_patches/badCLIP.jpg')
